...
public class EhCachePublicServiceJdbcGetTest<T extends IdEntity<PK>,PK,DB extends PublicDaoImpl<T,PK>>  implements Busines<T,PK,DB>{
...
//	private ExecutionCache<T> ec = new RedisEHcache<T>();
	...
	public boolean get(QueryInfo qi, PublicAPI<T, PK, DB> api, Result<T> result,ArrayList<DispatchTask> list,int index) throws Exception {
		if(qi.getKeyName()!=null ){//代表使用缓存查询
			...
			EhAllCache cache =  CacheMagger.getEhCache(api.getTimeout());
			...
			if(obj != null){
				...
				synchronized (getLock(api.getClas().getName())) { //并发不高不会引起缓存雪奔所以锁的力度很细
					obj =  (T) cache.get(key);
					if(obj != null){
						System.out.println("拿到缓存:"+ai.getAndIncrement());
						result.setRows(obj);
